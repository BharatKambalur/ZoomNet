...
im_path = "test.png"
weights_file = "ZoomNet_Weights_100.hdf5"
...
def process_section(x1, y1, x2, y2, sec_id):
    ...
    if small_side >= 90: #TODO: Add configuration parameter for minimum short side length
        ...
        im_sec_reshaped = cv2.
        np.vstack((im_list_to_process,np.expand_dims(img_pre[x1:x2+1,y1:y2+1],axis=0)))  # Add image to queue
        np.vstack((subsec_bounds_list,[x1, y1, x2, y2]))
...
def split_image_to_array():
    ...
    sec_id = 0      # Reset the global section and subsecttion counters to 0 for a new image
    global_s_id = 0
    for point_n in range(len(points)-1):
        if small_side_index == 0:
            ...
            if point_n != 0:            # Overlapping middle sections
                process_section(s_side_half-s_sec_half, points[point_n]-l_sec_half,
                                s_side_half+s_side_half-1, points[point_n]+l_sec_half-1, sec_id)
                sec_id = sec_id + 1
                sec_subsec_list.append([])
        else:
            ...
            if point_n != 0:            # Overlapping middle sections
                process_section(points[point_n] - l_sec_half, s_side_half - s_sec_half,
                                points[point_n] + l_sec_half - 1, s_side_half + s_side_half - 1, sec_id)
                sec_id = sec_id + 1
                sec_subsec_list.append([])
...
im_list_to_process = np.empty((0,4)) # 4 X n List which stores the 'n' images to be fed through the network
